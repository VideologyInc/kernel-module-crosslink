...
function write_check() {
    ...
    for l in {0..20}; do
        if [[ -n "${has_i2c_serial}" ]]; then
            res=$(crosslink-serial.py -d /dev/links/crosslink_mipi_1 "$1")
        else
...
function Sony() {
    ...
    for _ in {0..50}; do
        if [[ -n "${has_i2c_serial}" ]]; then
            res=$(crosslink-serial.py -d /dev/links/crosslink_mipi_1 "81090400FF")
        else
...
if [[ -n "${has_i2c_serial}" ]]; then
    res=$(crosslink-serial.py -d /dev/links/crosslink_mipi_1 "81090002FF")
else
